      ...
      <div className='flex gap-1 bg-black/80 rounded-lg p-1.5 shadow-lg pointer-events-auto border border-white/10'>
        {hotbarItems.map((item, index) => (
          <div
            key={index}
            className={cn(
              'w-[60px] h-[60px] md:w-[60px] md:h-[60px] w-[40px] h-[40px] bg-black/60 border-2 border-white/20 rounded-md flex justify-center items-center relative cursor-pointer transition-all duration-200 overflow-hidden',
              !item && 'bg-black/40',
          ...
          >
            {item ? (
              <>
                ...
                <div className='absolute top-0.5 left-1 text-xs md:text-sm text-white/90 font-bold bg-black/30 px-1.5 py-0.5 rounded'>
                  {index + 1}
                </div>
                <button
                  className='absolute top-0 right-0 w-[14px] h-[14px] md:w-[18px] md:h-[18px] bg-red-500/70 text-white rounded-tr-md rounded-bl-md text-xs flex justify-center items-center cursor-pointer opacity-0 hover:bg-red-500/90 transition-opacity duration-200 p-0 group-hover:opacity-100'
